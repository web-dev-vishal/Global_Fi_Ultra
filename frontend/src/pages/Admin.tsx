        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#131D2E] border border-slate-700/50 rounded-xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold text-white">Metrics (24h)</h3>
            </div>
            {loading ? (
              <div className="space-y-2">{[...Array(5)].map((_, i) => <Skeleton key={i} className="h-9 rounded-xl" />)}</div>
            ) : metrics ? (
              <div className="space-y-2">
                <p className="text-[10px] text-slate-600 uppercase tracking-wide mb-3">Period: {metrics.period}</p>
                {Object.entries(metrics.metrics ?? {}).slice(0, 8).map(([k, v]) => (
                  <div key={k} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-800/40">
